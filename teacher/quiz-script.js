...
async function loadQuestions() {
    if (examMode) {
        // Check attempt limits before starting exam
        try {
            const response = await fetch('api/get_scores.php');
            if (response.ok) {
                const result = await response.json();
                const studentData = result.data.find(student =>
                    student.class === classSelected.replace('tin_hoc_', '') &&
                    student.student_code == studentSelected.split('. ')[1]?.split(' ')[0]
                );

                if (studentData) {
                    const attempts = examPart === 'TX1' ? studentData.tx1_attempts : studentData.tx2_attempts;
                    if (attempts >= 3) {
                        alert(`Bạn đã thi ${examPart} đủ 3 lần. Vui lòng liên hệ giáo viên để được reset.`);
                        window.location.href = 'exam.html';
                        return;
                    }
                }
            } else {
                console.warn('Could not check attempt limits due to API error');
            }
        } catch (error) {
...
// Internal function to load questions (reusable for retry)
async function loadQuestionsInternal() {
    if (examMode) {
        ...
        if (window.questionsByLesson) {
            ...
            const selectedNB = shuffleArray(nbQuestions).slice(0, Math.min(10, nbQuestions.length));
            const selectedTH = shuffleArray(thQuestions).slice(0, Math.min(10, thQuestions.length));
...
function showResults() {
    ...
    // Calculate score out of 10: (correct_answers / total_questions) * 10, rounded
    score = Math.round((correctAnswers / questions.length) * 10);

    // Update the results heading to show correct/total
    const resultsHeading = document.querySelector('#result-container h2');
    if (resultsHeading) {
        resultsHeading.textContent = `Kết quả: ${correctAnswers}/${questions.length}`;
    }

    // Update score display to show points
    scoreDiv.textContent = `Điểm số: ${score}đ`;
    quizContainer.classList.add('d-none');
    resultContainer.classList.remove('d-none');

    // Show/hide back to exam button based on exam mode
    if (examMode) {
        backToExamBtn.classList.remove('d-none');
    } else {
        backToExamBtn.classList.add('d-none');
    }
...
restartBtn.onclick = async () => {
    if (examMode) {
        // Check attempt limits before allowing retry
        try {
            const response = await fetch('api/get_scores.php');
            if (response.ok) {
                const result = await response.json();
    ...
    // Reload and randomize questions for retry
    try {
        await loadQuestionsInternal();
        // Reset quiz state
        ...
        console.error('Error reloading questions for retry:', error);
        alert('Không thể tải lại câu hỏi. Vui lòng thử lại sau.');
    }
};

// Back to exam button event handler
