...
?>
    ...
    <script>
        ...
        // Load students table from students.json
        async function loadStudents(classFilter = '') {
            try {
                const response = await fetch('../admin/students.json');
                ...
                if (students) {
                    ...
                    if (classFilter !== '') {
                        filteredStudents = students.filter(student => student.class_code === classFilter);
                    }
        ...
        // Initialize data from students.json
        document.getElementById('initBtn').addEventListener('click', async function() {
            if (confirm('Bạn có chắc muốn khởi tạo dữ liệu từ students.json?')) {
                try {
                    const response = await fetch('../admin/students.json');
                    const students = await response.json();

                    if (students) {
                        // Create student_score.json with basic student info, no scores
                        const studentScores = students.map(student => ({
                            student_code: student.student_code,
                            student_name: student.student_name,
                            class_code: student.class_code,
                            test_name: '', // Placeholder, to be updated with scores
                            attempts: 0,
                        ...
                        alert('Không thể tải dữ liệu từ students.json');
                    }
                } catch (error) {
