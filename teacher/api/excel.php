...
foreach ($classesToExport as $className) {
    ...
    $worksheet->setCellValue('B1', 'Mã học sinh');
    $worksheet->setCellValue('C1', 'Họ và tên');
    // Column D is intentionally left blank
    $worksheet->setCellValue('F1', 'TX1');
    $worksheet->setCellValue('G1', 'TX2');
    ...
    foreach ($classData[$className] as $student) {
        ...
        $worksheet->setCellValue('B' . $row, $studentCode);
        $worksheet->setCellValue('C' . $row, $fullName);
        // Column D is intentionally left blank
        $worksheet->setCellValue('F' . $row, $student['tx1_score'] !== null ? $student['tx1_score'] : '');
        $worksheet->setCellValue('G' . $row, $student['tx2_score'] !== null ? $student['tx2_score'] : '');
        $row++;
        $stt++;
    }
}
